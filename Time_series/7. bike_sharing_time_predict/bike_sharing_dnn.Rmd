...
```{r}
start_time <- lubridate::minute(Sys.time())
model <- train(count~., data=baked_train, method="glmnet", metric="RMSE",verbose=FALSE)
end_time <- lubridate::minute(Sys.time())
time <- (end_time - start_time)
print(time)
pred <- predict(model, baked_test)
...
```
```{r}
baked_train$datetime <-as.Date(baked_train$datetime)
baked_train$datetime <-as.Date(baked_train$datetime)
baked_train
```
...
response <- "count"
n_features <- length(setdiff(names(baked_train), "count"))
...
print(RMSE(test_h2o$count,pred) )
```
