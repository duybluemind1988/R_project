...
```{r}
table(as.data.frame(test_h2o)$Attrition)
table( as.data.frame(test_pred)$predict)
```
```{r}
# Confusion matrix, number of cases
table(as.data.frame(test_h2o)$Attrition, as.data.frame(test_pred)$predict)
```
```{r}
#install.packages("mltools")
library(mltools)
mcc(as.data.frame(test_pred)$predict,as.data.frame(test_h2o)$Attrition) # 0.4520577
```
```{r}
#install.packages("Metrics")
library(Metrics)
Metrics::f1(as.data.frame(test_h2o)$Attrition,as.data.frame(test_pred)$predict)
Metrics::precision(as.data.frame(test_h2o)$Attrition,as.data.frame(test_pred)$predict)
Metrics::recall(as.data.frame(test_h2o)$Attrition,as.data.frame(test_pred)$predict)
Metrics::auc(as.data.frame(test_h2o)$Attrition,as.data.frame(test_pred)$predict)
```

```{r}
confusionMatrix(as.data.frame(test_pred)$predict,as.data.frame(test_h2o)$Attrition,positive = "Yes")
```
```{r}
confusionMatrix(
  data = relevel(as.data.frame(test_pred)$predict, ref = "Yes"), 
  reference = relevel(as.data.frame(test_h2o)$Attrition, ref = "Yes")
)
```

Evaluate train data set
```{r}
h2o.performance(h2o_model,train=TRUE)
```

```{r}
table(as.data.frame(train_h2o)$Attrition)
```

```{r}
train_pred <-h2o.predict(h2o_model, newdata = train_h2o)
confusionMatrix(as.data.frame(train_pred)$predict,as.data.frame(train_h2o)$Attrition,positive = "Yes")
```
