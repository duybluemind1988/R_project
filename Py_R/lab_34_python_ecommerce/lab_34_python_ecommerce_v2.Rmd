...
X_embedded_df <-pd.DataFrame(X_embedded) # Cannot print or show
```
```{r}
#print(reticulate::py$X_embedded)
```
...
# NOT ggradar use in this r notebook
```
```{r}
...
## Customer History
```{r}
head(customer_habits_joined_tbl)
```
