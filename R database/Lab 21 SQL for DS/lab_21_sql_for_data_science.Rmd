---
title: "Lab 21 - SQL for Data Science"
...
con <- dbConnect(
    ...
    dbname ="/home/dnn/Data_science/Git/Business science/Database-SQL/lab_21_SQL_for_datascience/lab_21_SQL_for_datascience/home_loan_applications"
)
...
```{r}
tbl(con, "applications_train") %>% count(NAME_CONTRACT_TYPE, TARGET) %>% show_query()
```
...
```
```{r}
# Add collect can make it happen, but not fast
tbl(con, "applications_train") %>%
    collect() %>% 
    filter(str_detect(NAME_CONTRACT_TYPE, "Cash")) 
```
...
# ERROR due to limitation
#tbl(con, "applications_train") %>% 
#    filter(grepl("Cash",NAME_CONTRACT_TYPE))
```
...
source("llpro_bonus_machine_learning.R") # 31.9 s
```
